	...
	for _, d := range dicts {
		...
		for _, filePath := range d.srcPaths {
			...
			for scanner.Scan() {
				...
				if d.decodeCLetters {
					letters = cLetters[w[len(w)-1]]
				}

				if len(letters) == 0 {
					builder.AddWord(dict.Word{Word: parts[1], Translit: parts[0]})
				}

				for _, letter := range letters {
					builder.AddWord(dict.Word{Word: parts[1] + letter, Translit: parts[0] + letter})
				}
			}
